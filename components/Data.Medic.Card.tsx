...
function DataMedicCard({children, title, value, color }: InterfaceMedicData) {
  ...
  return (
    <>
      <div className="relative flex-grow border border-gray-300 dark:text-gray-200  rounded text-center py-8">
        <h2 className={className}>{value}</h2>
        <h4 className="inline text-gray-500 dark:text-gray-200 text-sm">{title}</h4>
        {children}
      </div>
    </>
  );
