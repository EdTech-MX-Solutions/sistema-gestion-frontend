...
export const TableDirectives = ({ directives }: TableDirectivesProps) => {
    ...
    const handleDegradarADirectivo = async (id: string) => {
        ...
        const response =
            await api.directivosCollection.executePatchAscenderProfesorADirectivo(
                token,
                accion,
                id
            );
        if (response.status === 200) {
            const response2 =
                await api.directivosCollection.executeGetProfessors(token);
